 ...
 * Responsável pelas estâncias, sessões e atualizações de tráfego do sistema.
 */
class Session {
    ...
    private function getTraffic() {
        ...
        $readSiteViews->ExeRead('ws_siteviews',null, "WHERE siteviews_date = :date", "date={$this->Date}");
        if ($readSiteViews->getRowCount()):
            $this->Traffic = $readSiteViews->getResultado()[0];
        endif;
    }

    //Verifica, cria e atualiza o cookie do usuárioF [ HELPER TRAFFIC ]
    ...
    private function BrowserUpdate() {
        $readAgent = new Read;
        $readAgent->ExeRead('ws_siteviews_agent',null, "WHERE agent_name = :agent", "agent={$this->Browser}");
        if (!$readAgent->getResultado()):
            $ArrAgent = ['agent_name' => $this->Browser, 'agent_views' => 1];
            $createAgent = new Create;
            $createAgent->ExeCreate('ws_siteviews_agent', $ArrAgent);
        else:
            $ArrAgent = ['agent_views' => $readAgent->getResultado()[0]['agent_views'] + 1];
    ...
    private function UsuarioUpdate() {
        ...
        if (!$userUpdate->getRowCount()):
            $readSes = new Read;
            $readSes->ExeRead('ws_siteviews_online', null, 'WHERE online_session = :onses', "onses={$_SESSION['useronline']['online_session']}");
